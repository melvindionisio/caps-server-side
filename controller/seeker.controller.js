...
exports.getSeekerProfile = (req, res) => {
   ...
   db.query(`SELECT * FROM boarding_house_seekers WHERE seeker_id = ? `, [seekerId], (err, result) => {
      if (!err) {
         res.send(result[0])
      } else {
         res.send({
...
exports.registerSeeker = async (req, res) => {
   ...
   const sqlInsert = 'INSERT INTO boarding_house_seekers (seeker_name, seeker_username, seeker_password) VALUE (?,?,?)'
...
exports.loginSeeker = (req, res) => {
   ...
   if (username && password) {
      db.query('SELECT * FROM boarding_house_seekers WHERE seeker_username = ?', [username], async (err, result) => {
         if (err) {
            res.send({
               code: 400,
         ...
         if (result.length > 0) {
            ...
            const validate = await bcrypt.compare(password, result[0].seeker_password)
...
exports.updateSeeker = (req, res) => {
   ...
   db.query(
      `UPDATE boarding_house_seeker SET seeker_name = ?, seeker_username = ?,seeker_password = ? WHERE seeker_id = ?`,
      [newName, newUsername, newPassword, seekerId],
      (err, result) => {
         if (!err) {
            res.send({
